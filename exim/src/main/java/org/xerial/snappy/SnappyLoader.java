...
//                setSnappyApi(new PureJavaSnappy());
            throw new RuntimeException("Property file states we should load pure snappy");
        }
        else {
            loadNativeLibrary();
    ...
    static File findNativeLibrary()
    {
        ...
        if (!hasNativeLib) {
            String errorMessage = String.format(
                "no native library is found for os.name=%s and os.arch=%s\npath=%s (exists=%s, official=%s)",
                OSInfo.getOSName(),
                OSInfo.getArchName(),
                snappyNativeLibraryPath + "/" + snappyNativeLibraryName,
                Files.exists(Paths.get(snappyNativeLibraryPath, snappyNativeLibraryName)),
                SnappyLoader.class.getResource(snappyNativeLibraryPath+"/"+snappyNativeLibraryName) != null
            );
