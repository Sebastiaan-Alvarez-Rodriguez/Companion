...
public class NoteStore {
    ...
    public static void insert(@NonNull Note note, boolean secure, @NonNull FragmentManager manager, @NonNull Context context, @NonNull FinishListener finishListener, @NonNull ErrorListener errorListener) {
        ...
        noteQuery.isUniqueInstanced(note.getName(), result -> {
            if (result == null) { // Unique-named note
                insertInternal(note, secure, manager, context, noteQuery, finishListener, errorListener);
     ...
     */
    public static void update(@NonNull Note note, String prevName, boolean secure, @NonNull FragmentManager manager, @NonNull Context context, @NonNull FinishListener finishListener, @NonNull ErrorListener errorListener) {
        final NoteQuery noteQuery = new NoteQuery(context);
        if (!prevName.equals(note.getName())) {
            noteQuery.isUniqueInstanced(note.getName(), result -> {
                if (result == null) { // Unique-named note
                    updateInternal(note, secure, manager, context, noteQuery, prevName, finishListener, errorListener);
                } else { // Another note with same name exists
                    showOverrideDialog(result, manager, new StoreCallback() {
                        @Override
                        public void onSuccess() {
    ...
    /** Inserts given note, with specified security: <code>true</code> means making note secure, <code>false</code> to not modify the note */
    ...
    /** Equivalent to calling {@link #updateInternal(Note, boolean, FragmentManager, Context, NoteQuery, String, FinishListener, ErrorListener)} with {@code null} as replace candidate */
