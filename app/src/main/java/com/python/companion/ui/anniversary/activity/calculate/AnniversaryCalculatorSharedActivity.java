...
public class AnniversaryCalculatorSharedActivity extends AppCompatActivity {
    ...
    private List<Anniversary> others;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ArrayList<AnniversaryContainer> items = getIntent().getParcelableArrayListExtra("chosen");
        others = items.stream().map(AnniversaryContainer::getAnniversary).collect(Collectors.toList());
        findViews();
        setupActionBar();
        prepareButtons();
        prepareList();
        ...
        amountView.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                for (int x = 0; x < fastAdapter.getItemCount(); ++x) {
                    ...
                    final int w = x;
                    AnniversaryUtil.futureIntertwinedInterval(item.getAnniversary(), together, others, userInterval, date -> {
    ...
    private void prepareList() {
        ...
        sortHandler = new AnniversaryCalculatorSortHandler.Builder()
                .setStrategy(getSharedPreferences(getString(R.string.cactus_preferences), Context.MODE_PRIVATE).getInt("CactusCalculatorSort", AnniversaryCalculatorSortHandler.SORT_DURATION))
    ...
    private void setListUpdates() {
        ...
        viewModel.getAnniversaries().observe(this, anniversaries -> {
            List<AnniversaryCalculatorItem> list = anniversaries.parallelStream().map(anniversary -> {
                try {
                    return new AnniversaryCalculatorItem(anniversary, AnniversaryUtil.futureIntertwinedInterval(anniversary, together, others));
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        @IdRes int id;
        switch (getSharedPreferences(getString(R.string.cactus_preferences), Context.MODE_PRIVATE).getInt("CactusCalculatorSort", AnniversaryCalculatorSortHandler.SORT_DURATION)) {
